...
{
    int returnAux = -1;
    char auxNombre[CANTCARACTERES];
    char auxEmail[CANTCARACTERES*20];
    eDestinatario* nuevoDestinatario;

    if(pFile != NULL && pArrayListDestinatario != NULL){
        //printf("\nentro a parsear\n");
        ...
        while(!feof(pFile)){
            ...
            nuevoDestinatario = destinatario_new();

            if(nuevoDestinatario !=NULL){
                //printf("se creo puntero para destinatario\n");

                destinatario_SetNombre(nuevoDestinatario, auxNombre);
                destinatario_SetEmail(nuevoDestinatario, auxEmail);
                destinatario_SetEstado(nuevoDestinatario, 1);

                  //printf("%s\n", nuevoDestinatario->nombre);
                  //printf("%s\n", nuevoDestinatario->email);

                returnAux = pArrayListDestinatario->add(pArrayListDestinatario, nuevoDestinatario); //(void*)
...
int depurarDestinatarios(ArrayList* this, ArrayList* this2){
    int returnAux = -1, i, j, flag=0, contador=0;
    eDestinatario* destAux;
    eDestinatario* destAux2;

    if(this != NULL && this2 != NULL){

        for(i=0; i< this2->size; i++){
            destAux2 =(eDestinatario*) al_get(this2, i);
            printf("\nListaNegra(i): %s\n", destAux2->nombre);
            //if(flag!=0){
            //    contador++;
            //}
            ...
            for(j=0; j<this->size; j++){
                destAux =(eDestinatario*) al_get(this, j);
                printf("\nLista(j): %s\n", destAux->nombre);
                flag=1;

                if(strcmp(destAux2->nombre, destAux->nombre) == 0){
                    destAux->estado = -1;
                    flag=0;
                    break;
                }
...
int armaSublista(ArrayList* this, ArrayList* subThis){
    ...
    for(i=0; i < this->size; i++){
        destAux =(eDestinatario*) al_get(this, i);
        //printf("\nLista(i): %s\n", destAux->nombre);

        if(destAux->estado == 1){
            returnAux = al_add(subThis, destAux);
        }
    }
...
void imprimeLista(ArrayList* this){
    ...
    for(i=0; i<this->size; i++){
        auxDestinatario = (eDestinatario*) al_get(this, i);
